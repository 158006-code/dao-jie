...
function spawnEnemyAt(G, typeKey, x, y, enemyPhase){
  ...
  G.enemies.push({
    ...
    _bubble: def.quotes && Math.random()<({'roller':1,'berserker':1,'rich':1,'lazy':1,'brute':1}[def.key]?0.80:0.20) ? { text: def.quotes[Math.floor(Math.random()*def.quotes.length)], timer: 180 } : null,
  });
}
